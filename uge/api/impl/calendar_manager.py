...
class CalendarManager(DictBasedObjectManager):
    QCONF_ERROR_REGEX_LIST = [
        (re.compile('.*is still referenced in.*'), InvalidRequest),
        (re.compile('.*error in disabled_.*'), InvalidRequest),
        (re.compile('.*does not exist.*'), ObjectNotFound),
        (re.compile('.*is not a calendar.*'), ObjectNotFound),
        (re.compile('.*no calendar defined.*'), ObjectNotFound),
    ]

    # Failure incorrectly classified as successful outcome
    QCONF_FAILURE_REGEX_LIST = []

    GENERATE_OBJECT_FACTORY_METHOD = QconfObjectFactory.generate_calendar
    OBJECT_NAME_KEY = 'calendar_name'
    OBJECT_CLASS_NAME = 'Calendar'
    OBJECT_CLASS_UGE_NAME = 'cal'
    ...
    def get_bulk_dump_filename(self, object):
        return 'conf_api_dump_' + object.data['calendar_name']
