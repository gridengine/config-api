...
class QconfExecutor(object):
    ...
    def execute_qconf(self, cmd, error_regex_list=[], error_details=None, combine_error_lines=False, success_regex_list=[], failure_regex_list=[]):
        try:
            command = '. %s/%s/common/settings.sh; qconf %s' % (self.env_dict['SGE_ROOT'], self.env_dict['SGE_CELL'], cmd)
