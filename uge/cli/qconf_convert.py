...
class QconfConvert(QconfCli):
    ...
    def __init__(self):
        ...
        self.api = QconfApi()
        self.add_option('', '--input-file', dest='input_file', help='Input file containing object\'s JSON representation.')
        self.add_option('', '--to-uge', dest='target_uge_version', default=self.api.get_uge_version(), help='Target UGE version (default: %s).' % self.api.get_uge_version())
        self.add_option('', '--output-format', dest='output_format', default='json', help='Output format, either json or uge (default: json).')
